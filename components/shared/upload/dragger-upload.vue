  ...
  <div>
    ...
    >
      ...
      <template v-for="item in fileList" :key="item.uid">
        ...
        <span v-if="item.status === 'uploading'" class="absolute inset-0 flex items-center justify-center bg-slate-700/60 dark:bg-slate-500/60">
          <UIcon v-if="item.status === 'uploading'" name="i-lucide-loader" class="h-4 w-4 animate-spin" /> </span>
      </template>
